 ...
 */
export function getDomainKnowledge(url) {
    if (!url)
        return [];
    try {
        const hostname = new URL(url).hostname.toLowerCase();
        return DOMAIN_KNOWLEDGE.filter((entry) => hostname === entry.domain || hostname.endsWith("." + entry.domain));
    }
    catch {
        return [];
    }
 ...
 */
export function isAntiBotDomain(url) {
